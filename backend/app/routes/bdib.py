...
def _sid() -> str:
    if "_bdib_sid" not in session:
        session["_bdib_sid"] = secrets.token_hex(8)
    return session["_bdib_sid"]
...
def create_bdib_blueprint(*, vectorstores):
    ...
    def display_result_bdib(user_name):
        ...
        return render_template('answer_bdib.html', user_name=user_name, chat_history=_get_history())
    ...
    def rating_submission_bdib():
        if request.method == "POST":
            ...
            return render_template('answer_bdib.html', user_name=user_name, question=question, answer=answer, chat_history=_get_history())
